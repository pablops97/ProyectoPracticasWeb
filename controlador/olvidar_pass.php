...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $details = mysqli_query($db, "SELECT email FROM tecnico WHERE email='$email_reg'");
    if (mysqli_num_rows($details) > 0) {
        ...
        try {
            ...
            $mail->Password = 'exlf nylb prsf qmlv';
            $mail->SMTPSecure = PHPMailer::ENCRYPTION_STARTTLS;
            $mail->Port = 587; // Puerto SMTP
            $mail->setFrom('proyectofinciclodam@gmail.com', 'Pablo developer');
            $mail->addAddress($email_reg);
            $mail->Subject = 'Nueva password APP PROYECTO';
            $htmlMsg = '<a href="http://localhost/login/controlador/olvidar_pass_reset.php?key=' . $key . '&email=' . $email_reg . '">Enlace</a>';
            $mail->Body = "Pulse " . $htmlMsg . " para continuar con el proceso de cambio de contraseña";
            $mail->isHTML(true);
            $mail->send();
        } catch (Exception $e) {
